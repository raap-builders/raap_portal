...
const convertToInternationalCurrencySystem = (labelValue: number) => {
  return parseFloat((Math.abs(Number(labelValue)) / 1.0e6).toFixed(2));
};
...
const calculateIncrementalRevenue = (rooms: number) => {
  ...
  let financialSavings =
    financialSavingPerMonth * (projectTimeOnSite - projectTimeAiota);
  let occupancy = 0.55; //55%
  let adr = 125; //$125
  let additionalRoomRevenues =
    rooms * adr * occupancy * 30 * (projectTimeOnSite - projectTimeAiota);
